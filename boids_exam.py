...
class Boid:
    ...
    def _random_steer(self, spread: float = 0.2) -> None:
        ...
        if random.random() < 0.1:
            angle: float = random.uniform(-spread, spread)
            new_vx: float = self.vx * math.cos(angle) - self.vy * math.sin(angle)
            new_vy: float = self.vx * math.sin(angle) + self.vy * math.cos(angle)
            self.vx = new_vx
            self.vy = new_vy
    ...
    def _separation(self, boids: List['Boid']) -> pygame.Vector2:
        ...
        for other in boids:
            if other is not self:
                dist: float = math.hypot(self.x - other.x, self.y - other.y)
    ...
    def _alignment(self, boids: List['Boid']) -> pygame.Vector2:
        ...
        for other in boids:
            if other is not self:
                dist: float = math.hypot(self.x - other.x, self.y - other.y)
                if 0 < dist < config.ALIGNMENT_DISTANCE:
                    steer.x += other.vx
                    steer.y += other.vy
                    count += 1
